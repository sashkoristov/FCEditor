...
The following table shows the \texttt{mxCell}'s shapes which are implemented and used for composing a workflow. Beside a short description, the type of the associated user object is denoted.
...
\hline
\makecell{\includegraphics[width=32pt,height=24pt]{cell-merge} \\ \textsf{Merge}}  & For merging previous branches & \makecell{\texttt{null} \\ \small(None)} \\
\hline
...
Graph validation is implemented at two different stages: At runtime, when composing a workflow by adding and connecting the vertices, and before export, when saving the current graph. 
In AFCL, a valid workflow represented by a DAG always fulfills the following constraints:
\begin{itemize}
	\item There exists only one starting point
	\item Start has only one outgoing edge
	\item There exists only one termination point
	\item End has only one incoming edge
	\item There exist no cycles
	\item Functions have only one incoming and only one outgoing edge
	\item IfThenElse have exactly two outgoing edges (then, else)
	\item ...
\end{itemize}
