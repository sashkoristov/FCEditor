...
\todo[inline]{The abstract does not show why do we need a GUI to create workflow. I would say that domain experts who know the business, are usually not familiar with serverless technology, thereby they would have difficulties to develop the workflow with the API. Manual is slow, so we need some toolkit to facilitate workflow development... Somehow like this.}
...
%\todo[inline]{FCs by default are at a high-level of abstraction. We need a tool that will facilitate its development - focused on business logic}

\todo[inline]{write weaknesses of existing work here (visualization, without development options, which is still manual!), then after the sentence "This thesis ..." you will write our solution and how do you advance compared to state of the art.}
...
\todo{I would mention concrete problem and concrete solution. First both problems of SOTA - visualization only, without development, which is manual in text file or in JS, which requires development skills, or needs to adapt the workflow to be able to be executed across multiple pools of resources (e.g. multitple regions or providers - beyond the scope of this thesis.}

\todo[inline]{Why this service is important? Because AFCL has a limit to specify the location of each function. In order to allow a loop to be executed across multiple regions / providers, we need to unroll the loop and specify function locations within a loop. This is missing, which is very important why your work is important. Mention in provider limits in Section 2 that their languages are limited to run workflows within a single region.}
...
\todo[inline]{Better name chapter numbers.}
...
First of all, the serverless concept, in particular Function-as-a-Service, Serverless Functions and Workflows \todo{Can you switch the term into function choreographies? Define once in introduction and use the abbreviation FC instead of workflow.} are explained, before giving a brief overview about AFCL. The tools and technologies used for development are also presented to readers who are not yet familiar with them.
...
Serverless functions - or tasks - are single-purpose, mostly stateless, programmatic functions that are hosted on cloud infrastructure. These \todo{This} infrastructure is provided through a FaaS platform. They are event-driven and can be invoked through the internet, mainly using HTTP. Like conventional functions, they accept arguments and and return the result of the computation  - tthe difference being that this is done over a network.
...
\todo[inline]{I would not mention other works than AFCL, because you do not support those patterns in your toolkit. Just citation \cite{thesis-design-serverless-worfklow-system-eyk} for global control and data flow.}
...
\todo[inline]{introduce here how Step, IBM and others create workflows and that the users are unable to port from one to another FC system.}
...
\todo[inline]{The following paragraph should be moved to the previous section 2.4. Here write only about positive issues of AFCL, because your work is based on it, so based on a good language :)}
...
\chapter{AFCL ToolKit \todo{system architecture and design?}}
...
\todo[inline]{In general, this is too short section. Explain all white boxes, as well as communication with external stakeholders (user, scheduler, ...)}
...
\todo[inline]{refer to Fig. 3.1. Use the same terminology in the figure. Web Frontend - AFCLToolkit Frontend, while instead of Java Backend - use AFCLToolkit Backend.}
...
\todo[inline]{mention access to the frontend is through a browser.}
...
The frontend displays and controls the application's user interface. Its core component is the workflow editor, which is responsible for visualizing workflow data and provides a drawing area, allowing the user to compose and edit workflows in WYSIWYG manner.\footnote{https://en.wikipedia.org/wiki/WYSIWYG}\todo{can you please find some paper or link of report, rather than wikipedia?}



The editor\todo{it seems that core application is the main part as it communicates with all other components. Maybe you do not need to present core app, but the editor? If the editor loads and stores, then shouldn't there be a link to the web browser?} allows the user to load, create, edit and save AFCL workflows (YAML or JSON), which visualizes the current workflow as a graph. While editing, actions are constantly validated to a set of constraints which ensures workflows validity. The editor includes an action history which enables undo and redo, as well as a clipboard for copy and paste.

\todo[inline]{Where are load, export on the architecture? Encoding and decoding should be explained in the text, but user does load and store / export.}
...
The main purpose of the backend is to perform operations on workflows at low-level. These operations include the conversion of XML\todo{no one knows at this stage about XML representation. Either you explain in the frontend, or use here intermediary representation. I prefer the former.} data delivered from the frontend, as well as complex workflow modifications.
For the communication with the world outside\todo{show this in the figure}, the backend includes a web server\todo{isn't server a deployment issue? here should be a web service?}, which is used to expose the endpoint for its HTTP-based API to clients. Moreover, the frontend web application is also delivered by this web server. 
...
\todo[inline]{My main concern about this section is that it tries to explain how did you do the programming, but not how did you solve the functionalities that are presented in Section 3.1. Let me give you an example. You explain the system architecture of a car and you say: I used tires from Goodyear, then wheel from China. Instead, you need to explain how the car moves left when you move the wheel to the left.}
...
\todo[inline]{Can you synchronize white boxes of the architecture figure (frontend) with this section? Only core application is a subsection, but libraries, graph and others are not visible in the figure.}
...
\todo[inline]{These libraries should be included in background. Here you should write what is their advantage vs. others so that you used them.}
\begin{itemize}
\item \textbf{React}\\
...
\todo[inline]{I think the following paragraph including Fig. 3.5 should go in system architecture for the backend (What did you do?). Here you write detauls how did you solve that with the three procedures (How did you do?)}
...
\todo[inline]{it will be nice to give an example of renaming the innner functions, adding prefixes/suffixes}

\chapter{Web Interface}
\label{chap:impl-webinterface}

\todo[inline]{Web interface is missing on the architecture figure.}
...
\todo[inline]{I guess the three components are functions, editor and settings, which are accessible through Dashboard. So, better not to put 4.1 dashboard.}
...
\todo[inline]{You need to connect functions in backend architecture in Section 3.1. That you use a repository so that the user can create a workflow from existing functions? How do you load a workflow if there are no existing functions? }
...
\todo[inline]{too short for a subsection. Give some examples of such constants, maybe also with a figure.}
...
\todo{put names for methods (manual, AFCL API, and AFCToolkit.}

\todo[inline]{Write some \% that your toolkit generates workflows the fastest. Explain why manual is much slower (e.g. in lines of code)}
...
\todo[inline]{STart here - the full benefit / contribution of the AFCL Toolkit is visible in FC adaptation. }
...
\todo[inline]{Can you estimate some equation to estimate the speedup? In case of more complex nesting in the loop, AFCL toolkit has few changes only, while other methods need to copy and update many.}
...
\todo[inline]{do not use separate section for future work. Either at least two or zero sections in the chapter. }
\section{Future Work}
